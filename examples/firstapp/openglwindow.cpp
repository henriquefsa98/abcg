...
void OpenGLWindow::paintUI() {
  ...
  // Our own ImGui widgets go below
  {
    ...
    ImGui::Checkbox("Se nao checar esta errado", &enabled);
    fmt::print("The checkbox is {}\n", enabled ? "enabled" : "disabled");

    if (!enabled){

        ImGui::Begin("Esta errado!");

        auto cornoWindowSettings{getWindowSettings()};

        ImGui::Text("Voce mander a checkbox selecionada!");
        ImGui::End();
    }
